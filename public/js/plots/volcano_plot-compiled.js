 ...
 */
VolcanoPlot = function VolcanoPlot(data, parent_div_id, settings) {
...
VolcanoPlot.prototype.initialization = function (settings) {
    ...
    self.xscale = d3.scale.linear().domain([Math.log2(self.settings.r_min_limit), Math.log2(self.settings.r_max_limit)]).range([self.chart_x, self.chart_x + self.chart_width]);
    self.yscale = d3.scale.linear().domain([0, Math.log10(self.p_min_limit) * -1]).range([self.margin.top + self.chart_height, self.chart_y]);
    self.size_scale = d3.scale.log().domain([1, 600]).range([3, 10]);
};
...
VolcanoPlot.prototype.render = function (input_data, settings) {
    ...
    function redraw() {
        //放大缩小 鼠标滑动   scale放大倍数，translate是转变，转换
    ...
    var zoom = d3.behavior.zoom().x(self.xscale).y(self.yscale).scaleExtent([0.5, 20]).on("zoom", redraw);
    var container = d3.select("#" + self.parent_div_id);
    container.selectAll('svg').remove();
    var cur_svg = container.append('svg').attr("id", self.parent_div_id + "_volcano_svg").attr("class", 'volcano svg').attr("width", self.settings.width).attr("height", self.settings.height).attr("overflow", "hidden").call(zoom);

    var svg_g = cur_svg.append("g"); //Important for Zoom in out

    var masked_svg_g = cur_svg.append("g") //Important for Zoom in out
    .attr("clip-path", "url(#maskurl)");

    cur_svg.append("defs").append("svg:clipPath").attr("id", "maskurl").append("rect").attr('width', self.chart_width).attr('height', self.chart_height).attr('x', self.margin.left).attr('y', self.margin.top);

    svg_g.append("rect").attr("class", "overlay").attr("width", self.chart_width).attr("height", self.chart_height).attr("fill", 'white').style("pointer-events", "all");
...
VolcanoPlot.prototype.draw_axis = function (svg_g) {
    ...
    self.xAxis_1 = d3.svg.axis().scale(self.xscale).orient("bottom").ticks(5);

    self.yAxis = d3.svg.axis().scale(self.yscale).orient("left").ticks(5);

    self.yAxis_1 = d3.svg.axis().scale(self.yscale).orient("left").ticks(5);
    //Create axises
    // Add the X Axis
    svg_g.append("g").attr("class", "x_axis")
    //.attr("transform", "translate("+cur_margin.margin_left+","+(height+cur_margin.margin_top)+")")
    .attr("transform", "translate(" + 0 + "," + (self.margin.top + self.chart_height) + ")").call(self.xAxis_1).selectAll("path").attr("fill", "none");
    //.selectAll("text")
    ...
    // Add the Y Axis
    svg_g.append("g").attr("class", "y_axis")
    //.attr("transform", "translate(" + 0 + "," + 0 + ")")
    .attr("transform", "translate(" + self.margin.left + "," + 0 + ")").call(self.yAxis_1).selectAll("path").attr("fill", "none");

    //Add grid
    svg_g.append("g").attr("class", "x axis grid").attr("transform", "translate(" + 0 + "," + (self.margin.top + self.chart_height) + ")").attr({
        'fill': 'none',
        'stroke': 'grey',
        'opacity': 0.6
    }).call(self.xAxis.tickSize(-self.chart_height, 0, 0).tickFormat(""));

    svg_g.append("g").attr("class", "y axis grid").attr("transform", "translate(" + self.margin.left + "," + 0 + ")").attr({
        'fill': 'none',
        'stroke': 'grey',
        'opacity': 0.6
    }).call(self.yAxis.tickSize(-self.chart_width, 0, 0).tickFormat(""));
...
VolcanoPlot.prototype.draw_axis_title = function (svg_g) {
    ...
    .attr("x", self.margin.left + self.chart_width / 2).attr("y", self.margin.top + self.chart_height + self.margin.bottom * 2 / 3).style("fill", "black").style("text-anchor", "middle").text(function (d) {
    ...
    .append("text").attr("y", self.margin.left / 3).attr("x", -1 * (self.margin.top + self.chart_height / 2)).style("fill", "black").style("text-anchor", "middle").text(function (d) {
...
VolcanoPlot.prototype.draw_title = function (svg_g) {
    ...
    if (self.settings.title != undefined && self.settings.title != "") {
        ...
        svg_g.append("g").append("text")
        //.selectAll(".multi_line_type_Labelg")
        .attr("x", self.margin.left + self.chart_width / 2).attr("y", self.margin.top / 2).style("fill", "black").style("text-anchor", "middle").style("font-family", "sans-serif").style("font-size", "14px").text(function (d) {
...
VolcanoPlot.prototype.draw_legends = function (svg_g) {
    var self = this;
    //Draw zero line
    svg_g.append("line").attr("id", "volcano_hr_zero_line").attr("class", "redrawable").attr("y1", self.margin.top).attr("x1", self.xscale(0)).attr("y2", self.margin.top + self.chart_height).attr("x2", self.xscale(0)).attr("stroke", "blue").attr("opacity", '0.8').attr('stroke-width', 2);
    //Draw p 0.05 line
    svg_g.append("line").attr("id", "volcano_hr_zero_line").attr("class", "redrawable").attr("y1", self.yscale(Math.log10(0.05) * -1)).attr("x1", self.margin.left).attr("y2", self.yscale(Math.log10(0.05) * -1)).attr("x2", self.margin.left + self.chart_width).attr("stroke", "red").attr("opacity", '0.8').attr('stroke-width', 2);

    svg_g.append("text").attr("id", "volcano_hr_zero_line").attr("class", "redrawable").attr("y", self.yscale(Math.log10(0.05) * -1) - 10).attr("x", self.margin.left + self.chart_width - 50).text("p = 0.05").attr("fill", "red").attr("stroke-width", 0);
};
...
VolcanoPlot.prototype.draw_serie_ls = function (svg_g) {
    ...
    for (var series_name in this.__data) {
        var series_g = seris_ls_g.append("g").attr("class", function (d) {
            return "volcano series " + series_name;
        }).attr("id", function (d) {
            return "volcano_series_" + series_name;
        });
...
VolcanoPlot.prototype.draw_series = function (parent_g, series) {
    ...
    parent_g.append("g").selectAll("circle").data(series).enter().append("circle").attr("class", function (d) {
        return "volcano series " + series.name + " circle " + d.id + " redrawable";
    }).attr("id", function (d) {
        return "volcano_series_" + series.name + "_circle_" + d.id;
    }).attr("cx", function (d) {
        var hr = d.r;
        ...
        return y_scale(Math.log10(Math.max(parseFloat(d.p), self.settings.p_limit)) * -1) + 3;
    }).attr("r", function (d) {
        if (self.settings.use_size) {
            return self.size_scale(d.value);
        } else {
            return self.settings.radio;
        }
    ...
    }).attr("opacity", 0.4).on("mouseover", function (d) {
        var cur_item = d3.select(this);
        var a = d3.select(this);
        a.attr("fill", "orange");
        a.attr("opacity", 0.8); //draw_biomarker_statistic_tooltip_2(d.biomarker,d.case_ls,d.data_filter_str);
...
VolcanoPlot.prototype.draw_series_text = function (parent_g, series) {
    ...
    parent_g.append("g").selectAll("text").data(series).enter().append("text").attr("class", function (d) {
        return "volcano series " + series.name + " text " + series.id + " redrawable";
    }).attr("id", function (d) {
        return "volcano_series_" + series.name + "_text_" + series.id;
    }).attr("x", function (d) {
        var hr = d.r;
        if (hr < self.settings.r_min_limit) {
            hr = self.settings.r_min_limit;
        ...
        return y_scale(Math.log10(Math.max(parseFloat(d.p), self.settings.p_limit)) * -1) - 12;
    }).text(function (d) {
        if (d.p < 0.1) {
            if (d.name.length > 10) {
...
VolcanoPlot.prototype.draw_legends = function (svg_g) {
    var self = this;
    //Draw zero line
    svg_g.append("line").attr("id", "volcano_hr_zero_line").attr("class", "redrawable")
    //.attr("y1", self.margin.top)
    .attr("y1", -1000).attr("x1", self.xscale(0))
    //.attr("y2", self.margin.top + self.chart_height*)
    .attr("y2", self.margin.top + self.chart_height * 3).attr("x2", self.xscale(0)).attr("stroke", "blue").attr("opacity", '0.8').attr('stroke-width', 2);
    //Draw p 0.05 line
    svg_g.append("line").attr("id", "volcano_hr_zero_line").attr("class", "redrawable").attr("y1", self.yscale(Math.log10(0.05) * -1))
    //.attr("x1", self.margin.left)
    .attr("x1", -1000).attr("y2", self.yscale(Math.log10(0.05) * -1))
    //.attr("x2", self.margin.left + self.chart_width)
    .attr("x2", self.margin.left + self.chart_width * 3).attr("stroke", "red").attr("opacity", '0.8').attr('stroke-width', 2);

    svg_g.append("text").attr("id", "volcano_hr_zero_line").attr("class", "redrawable").attr("y", self.yscale(Math.log10(0.05) * -1) - 10).attr("x", self.margin.left + self.chart_width - 80).text("p = 0.05").attr("fill", "red").attr("stroke-width", 0);
};
...
VolcanoPlot.prototype.draw_sunburst_tooltip = function (parent_div, data, html_str, title) {
    ...
    if (data != undefined) {
        ...
        if (tooltip == undefined || tooltip[0].length < 1) {
            tooltip = d3.select("body").append('div').attr("id", "cur_sunburst_km_tooltip").attr("class", "sunburst_km_tooltip").style('display', 'none').style('z-index', 999999999);
        } else {
        ...
        if (data != undefined) {
            ...
            tooltip.style("left", d3.event.pageX + 7 + "px").style("top", d3.event.pageY - 40 + "px").classed('hidden', false).style('position', 'absolute');
        }
...
function setTooltipLocation(parent_div, tooltip, type, distance) {
    ...
    tooltip.style("left", x + "px").style("top", y + "px").style('position', 'absolute').moveNodeToFront();
}

//# sourceMappingURL=volcano_plot-compiled.js.map